...
export class AuthService {
    ...
    async login(email: string, plainPassword: string, req: RequestInfo): Promise<LoginResDto> {
    ...
    async logout(accessToken: string, refreshToken: string): Promise<void> {
        ...
        await Promise.all([
            this.addToBlacklist(accessToken, jtiAccess, 'access', 'ACCESS_TOKEN_EXPIRY'),
            this.addToBlacklist(refreshToken, jtiRefresh, 'refresh', 'REFRESH_TOKEN_EXPIRY'),
        ]);
    }

    async refreshAccessToken(refreshToken: string): Promise<string> {
        try {
            const { exp , ...payload } = await this.jwtService.verifyAsync(refreshToken, {
                secret: this.configService.get<string>('JWT_SECRET'),
            });
    ...
    async createAccessToken(user: User, payload: TokenPayload): Promise<string> {
        ...
        await this.accessTokenRepository.saveAccessToken(payload.jti, user, token, expiresAt);
    ...
    async createRefreshToken(user: User, payload: TokenPayload): Promise<string> {
        ...
        await this.refreshTokenRepository.saveRefreshToken(payload.jti, user, token, expiresAt);
    ...
    private async addToBlacklist(
        ...
        const expiryTime = this.calculateExpiry(this.configService.get<string>(expiryConfigKey));
        await this.tokenBlacklistService.addToBlacklist(token, jti, type, expiryTime);
    }
